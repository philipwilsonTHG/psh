...
class Parser(BaseParser):
    ...
    def parse_statement(self) -> Optional[Statement]:
        ...
        # Special handling for arithmetic commands that might be part of and_or_list
        if self.match(TokenType.DOUBLE_LPAREN):
            arith_cmd = self.parse_arithmetic_command()
            
            # Check if followed by && or ||
            if self.match(TokenType.AND_AND, TokenType.OR_OR):
                # Create a pipeline with the arithmetic command
                pipeline = Pipeline()
                pipeline.commands.append(arith_cmd)
                
                # Create and_or_list and continue parsing
                and_or_list = AndOrList()
                and_or_list.pipelines.append(pipeline)
                ...
                # Just the arithmetic command by itself
                return arith_cmd
