"""Refactored parser with improved organization and structure."""
...
class Parser(BaseParser):
    """Main parser class with improved organization.
    
    The parser is organized into logical sections:
    1. Top-level parsing - Entry points and high-level structure
    2. Statement parsing - Command lists and statements
    3. Command parsing - Simple commands, pipelines, and-or lists
    4. Control structures - If, while, for, case, select
    5. Expressions - Test expressions and arithmetic
    6. Functions - Function definitions
    7. Redirections - I/O redirection parsing
    8. Utilities - Helper methods and token management
    """
    ...
    def _at_command_list_end(self) -> bool:
        ...
        return self.match_any(TokenGroups.COMMAND_LIST_END)
    ...
    def parse_command(self) -> SimpleCommand:
        ...
        if self.match_any(TokenGroups.CASE_TERMINATORS):
            raise self._error(f"Syntax error near unexpected token '{self.peek().value}'")
    ...
    def _parse_case_pattern(self) -> str:
        ...
        if not (self.match(TokenType.WORD, TokenType.STRING, TokenType.VARIABLE) or
                self.match_any(TokenGroups.CASE_PATTERN_KEYWORDS)):
