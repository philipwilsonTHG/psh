...
- **Command Substitution in Arithmetic**: The arithmetic parser doesn't support command substitution inside arithmetic expressions. For example, `$(($(get_number) * 2))` fails because the parser encounters `$` which it doesn't recognize. The arithmetic parser only supports variable names without the `$` prefix.
...
- ~~**Variables Embedded in Words**~~ ✅ FIXED (v0.28.1): The state machine lexer now properly tracks embedded variables in words using RichToken metadata. The expansion system was updated to expand variables in WORD tokens that contain `$`. Example: `pre$x` now correctly expands to `pretest` when `x=test`.
- ~~**COMPOSITE Arguments Lose Quote Information**~~ ✅ PARTIALLY FIXED (v0.28.1): As a conservative fix, glob expansion is now disabled for all COMPOSITE arguments to prevent incorrect expansion of quoted wildcards. This ensures `file'*'.txt` outputs `file*.txt` instead of expanding the wildcard. A complete fix would require the lexer to produce single tokens for adjacent parts.
